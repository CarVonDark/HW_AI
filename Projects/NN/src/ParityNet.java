...
public class ParityNet {
	...
	public void initNetwork(double[][] inputs, double[][] desiredOutput) {
		...
		for (int i = 0; i < this.hiddenLayerSize; i++) {
			for (int j = 0; j < this.hiddenLayerSize; j++) {
				this.weights[0][i][j] = 1;
			}
		}
		for (int j = 0; j < this.hiddenLayerSize; j++) {
			this.weights[1][j][0] = Math.pow(-1, j);
		}
		this.bias = new double[] { -6, -5, -4, -3, -2, -1, 0 };
	}
		...
		for (int i = 0; i < this.hiddenLayerSize; i++) {
			for (int j = 0; j < this.outputLayerSize; j++) {
				System.out.printf("Weight from last hidden layer node %d to output node %d is %f.\n", i, j,
						this.weights[this.numHiddenLayers][i][j]);
			}
		}
	}
	...
	public void testNetwork() {
		...
		for (int example = 0; example < this.trainingSetSize; example++) {
			// We store the activation of each node (over all input and hidden layers)
			// because we need that data during back propagation.
			// Please notice that in order to simplify the code, we store the activation of
			// the input layer too.
			...
			// There may be different sizes for the input, hidden and output layers, hence
			// there are three different calls for feedForeard
			...
			for (int j = 0; j < this.outputLayerSize; j++) {
				if (Math.abs(activation[this.numHiddenLayers + 1][j] - this.desiredOutput[example][j]) > 0.1) {
					...
					for (int i = 0; i < inputLayerSize; i++) {
						System.out.print(activation[0][i] + " ");
					}
					System.out.println();
					System.out.println("Output neuron " + j + " has: " + activation[this.numHiddenLayers + 1][j]
							+ " should be: " + this.desiredOutput[example][j]);
					count++;

...
public class XOR {
	...
	private static void printWeights() {
		...
		for (int i = 0; i < inputLayerSize; i++) {
			for (int j = 0; j < hiddenLayerSize; j++) {
				System.out.printf("Weight from input %d to hidden %d is %f. \n", i, j, hiddenLayerWeights[i][j]);
			}
		}
		System.out.println("\nOutput layer");
		for (int j = 0; j < hiddenLayerSize; j++) {
			System.out.printf("Weight from hidden %d is %f. \n", j, outputLayerWeights[j]);
		}
		System.out.println();
	}
